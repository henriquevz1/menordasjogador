...
export const Route = createFileRoute("/")({
  head: () => ({ meta: [
    { title: "MENOR JOGADOR — Calças e Bermudas Jeans Masculinos" },
    { name: "description", content: "Moda masculina urbana com calças e bermudas jeans. Escolha seu modelo e compre diretamente pelo WhatsApp." },
    { property: "og:title", content: "MENOR JOGADOR — Estilo que veste a quebrada" },
    { property: "og:description", content: "Calças e bermudas jeans masculinos com atitude, qualidade e identidade." },
...
const products: Product[] = [
  ...
  { name: "Bermuda Jogador Black Gold Splash", category: "Bermudas Jeans", image: blackGoldSplashShortsAsset.url, description: "Modelo preto destroyed com respingos brancos, barra desfiada e detalhes amarelos.", washes: ["Preto intenso"], price: "R$ 135,00" },
];

function ProductCard({ product }: { product: Product }) {
  const [size, setSize] = useState("");
  const message = `Olá! Vim pelo site da Menor Jogador e quero saber mais sobre o produto ${product.name}${size ? `, tamanho ${size}` : ""}.`;
  return (
...
function HomePage() {
  return (
    <SiteLayout>
      <section className="relative min-h-[calc(100svh-5rem)] overflow-hidden bg-secondary text-secondary-foreground sm:min-h-[calc(100svh-6rem)]">
        <div className="pointer-events-none absolute inset-0">
          <img src={arrudaLogoUrl} alt="Logo MENOR JOGADOR — Menó das Jogador" width={1254} height={1254} className="h-full w-full object-cover opacity-40" />
        </div>
        <div className="image-shade absolute inset-0 bg-gradient-to-r from-secondary via-secondary/85 to-secondary/55" />
        <div className="relative mx-auto flex min-h-[calc(100svh-5rem)] max-w-7xl items-end px-4 pb-14 pt-28 sm:min-h-[calc(100svh-6rem)] sm:px-6 sm:pb-20 lg:items-center lg:px-8 lg:py-24">
          <div className="max-w-3xl">
            <p className="mb-4 text-xs font-bold uppercase tracking-[0.3em] text-primary">Denim • São Paulo</p>
            <h1 className="text-[clamp(4.5rem,16vw,10rem)] font-black uppercase leading-[0.72] drop-shadow-lg">Menor<br /><span className="text-primary">Jogador</span></h1>
            <p className="mt-7 text-xl font-semibold sm:text-2xl drop-shadow">Estilo que veste a quebrada.</p>
      ...
      <section className="px-4 py-16 sm:px-6 sm:py-24 lg:px-8"><div className="mx-auto max-w-7xl"><div className="grid gap-10 lg:grid-cols-2"><div><p className="text-xs font-bold uppercase tracking-[0.25em] text-primary">Recomendações dos clientes</p><h2 className="mt-3 text-5xl font-black uppercase leading-[0.9]">O que nossos clientes dizem</h2></div><div className="border border-dashed border-border bg-card p-8"><div className="flex gap-1 text-primary">{[1,2,3,4,5].map(n => <Star key={n} className="h-4 w-4" />)}</div><p className="mt-6 font-display text-2xl font-bold uppercase">Este espaço aguarda avaliações reais.</p><p className="mt-3 text-sm leading-6 text-muted-foreground">A MENOR JOGADOR poderá publicar aqui comentários autorizados de clientes. Nenhum depoimento demonstrativo foi apresentado como real.</p><Button asChild variant="outline" size="xl" className="mt-6"><a href={whatsappUrl("Olá! Vim pelo site da Menor Jogador e gostaria de enviar uma avaliação sobre minha compra.")} target="_blank" rel="noreferrer">Enviar avaliação <Check /></a></Button></div></div></div></section>
