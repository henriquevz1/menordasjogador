...
export const generalMessage = "Olá! Vim pelo site da Menor Jogador e quero saber mais sobre os produtos.";
  ...
  return (
    <footer className="bg-secondary text-secondary-foreground">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-[1.4fr_1fr_1fr] lg:px-8">
        <div><Wordmark light /><p className="mt-5 max-w-sm text-sm text-secondary-foreground/65">Calças • Bermudas • Moda Masculina<br />Estilo que veste a quebrada.</p></div>
        <div>
      ...
      <div className="border-t border-secondary-foreground/10 px-4 py-5 text-center text-[0.68rem] uppercase tracking-widest text-secondary-foreground/40">© 2026 MENOR JOGADOR. Todos os direitos reservados.</div>
    </footer>
  );
}
...
export function WhatsAppFloat() {
  return (
    <a href={whatsappUrl()} target="_blank" rel="noreferrer" aria-label="Falar com a Menor Jogador pelo WhatsApp" className="fixed bottom-4 right-4 z-50 grid h-14 w-14 place-items-center rounded-full bg-whatsapp text-whatsapp-foreground shadow-xl transition-transform hover:scale-105 sm:bottom-6 sm:right-6">
...
export function LegalPage({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children: ReactNode }) {
  return (
    <SiteLayout>
      ...
      <section className="border-t border-border px-4 py-10 sm:px-6"><div className="mx-auto flex max-w-5xl items-center gap-3 text-sm text-muted-foreground"><ShieldCheck className="h-5 w-5 shrink-0 text-primary" /><span>Em caso de dúvida, confirme as condições diretamente no canal oficial da MENOR JOGADOR.</span></div></section>
    </SiteLayout>
  );
}
